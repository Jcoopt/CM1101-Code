...
def remove_punct(text):
    ...
    """
    text_punct_rem=""
    for ch in text:# The pass statement does nothing. Replace it with the body of your function.
        if ch.isalpha() or ch ==" ":
            text_punct_rem="{0}{1}".format(text_punct_rem,ch)
    return text_punct_rem
...
def remove_spaces(text):
    ...
    for ch in text:
        if ch == " ":
            text_space_rem=text_space_rem[1:]
        else:
            break
...
def normalise_input(user_input):
    ...
    'help'
    """
    user_input_punct_rem=remove_punct(user_input)
    user_input_space_punct_rem=remove_spaces(user_input_punct_rem)
    normalised_input=user_input_space_punct_rem.lower()
...
def display_room(room):
    ...
    print("{2}{0}{2}{2}{1}{2}".format(room["name"].upper(),room["description"],"\n"))
...
def menu(exits):
    ...
    while True:
       ...
       # pass
        ...
        print_menu(exits)
        # Read player's input
        player_direction_request= input("> ")
        while not(is_valid_exit(exits,player_direction_request)):
            print("Sorry, I don't recognise that. Try again")
            print_menu(exits)

            player_direction_request = input("> ")
        return player_direction_request
